...
function parseMapIndex(mapidx) {
    ...
    for (const [key, value] of Object.entries(mapidx)) {
        ...
        let displayTitle =
            title + (title.toLowerCase().includes(key) ? "" : " (" + key + ")")
...
function makeTable(tableId, data) {
    ...
    sortedApis.forEach((api, i) => {
        ...
        if (base_url) {
            if (
                base_url.indexOf("mvt") === -1 &&
                base_url.indexOf("wms") === -1 &&
                base_url.indexOf("wfs") === -1
            ) {
                cell1_Naam.innerHTML +=
                    "<div class='icon-status-none' id='" +
                    api.short_name +
                    "_status'></div>"
            }
        }
        ...
        for (let urlName of Object.keys(api.api_urls)) {
            cell2_link.innerHTML +=
                '<a href="' + api.api_urls[urlName] + '">' + urlName + "</a> "
        }

        // Documentation column
        let cell4_Docs = row.insertCell(2)
        for (let urlName of Object.keys(api.documentatie_urls)) {
            cell4_Docs.innerHTML =
                '<a title="' +
                urlName +
                ' documentatie" href="' +
                api.documentatie_urls[urlName] +
                '">' +
                urlName +
                "</a>"
        }
        ...
        if (api.legacy === true) {
            cell5_Status.innerHTML =
                "Legacy API <b>Niet gebruiken</b>, wordt <b>verwijderd</b>"

            if (api.replacedBy) {
                cell5_Status.innerHTML +=
                    " Vervangende API: <a href='" +
                    api.replacedBy +
                    "'>" +
                    api.replacedBy +
                    "</a> "
            ...
            cell5_Status.innerHTML +=
                " </br><a href=#legacy-info>Meer informatie</a>"
        }
...
window.onload = () => {
    let promises = [
        JSONRequest("manual_apis.json").catch((e) => {
            console.log("Kan manual datasets niet ophalen.")
        }),
        JSONRequest(mapDomain + "/maps/index.json").catch((e) => {
            console.log("Kan index van " + mapDomain + " niet ophalen.")
        }),
        JSONRequest(domain + dsoPath + "?_format=json").catch((e) => {
            console.log("Kan datasets niet ophalen.")
        }),
        JSONRequest(domain + dsoPath + "wfs/").catch((e) => {
            console.log("Kan wfs datasets niet ophalen.")
        }),
        JSONRequest(domain + dsoPath + "mvt/").catch((e) => {
            console.log("Kan mvt datasets niet ophalen.")
        }),
    ]
    clearSearch()
    ...
    for (let table of Object.keys(tables)) {
        tables[table].forEach((row, i) => {
            if (row.naam.toLowerCase().includes(query)) {
                document
                    .getElementById(table + "-table-row-" + i)
                    .classList.remove("filter-hidden")
            } else {
                document
                    .getElementById(table + "-table-row-" + i)
...
window.addEventListener("load", async function () {
    ...
    try {
        ...
        for (const dataset in data.datasets) {
            apiUrls.push([
                data.datasets[dataset].short_name,
                data.datasets[dataset].versions[0].api_url,
            ])
        }
        ...
        for (const apiUrl of apiUrls) {
            try {
                ...
                const fullPaths = paths.map(
                    (path) => `${apiUrl[1]}${path}?_pageSize=1`
                )
